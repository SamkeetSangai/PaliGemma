...
class KVCache:
    ...
    def update(
        ...
        # Return all the new and existing keys
...
class GemmaRotaryEmbedding(nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, device=None):
        ...
        # Calculate the theta using formula theta_i = base^(-2i/dim) where i = 0, 1, 2, ..., dim // 2
        inv_freq = 1.0 / (
            self.base
...
def apply_rotary_pos_emb(q, k, cos, sin, unsqueeze_dim=1):
    ...
    # Apply Rotary Positional Encoding formula
...
class PaliGemmaForConditionalGeneration(nn.Module):
    ...
    def _merge_input_ids_with_image_features(
        ...
        # Combine the embeddings of the image tokens and the text tokens and
        ...
        if kv_cache is not None and kv_cache.num_items() > 0:
            # The position of the query is last position only
    ...
    def forward(
        ...
        # Extra the input embeddings [Batch_Size, Seq_Len, Hidden_Size]
        inputs_embeds = self.language_model.get_input_embeddings()(input_ids)

        # Merge text and images [Batch_Size, Channels, Height, Width] -> [Batch_Size, Num_Patches, Embed_Dim]
