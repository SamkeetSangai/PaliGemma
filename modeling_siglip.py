...
class SiglipAttention(nn.Module):
    ...
    def forward(
        ...
        if attn_weights.size() != (batch_size, self.num_heads, seq_len, seq_len):
            raise ValueError(
                f"Expected attention weights to have shape {(batch_size, self.num_heads, seq_len, seq_len)}, "
                f"but got {attn_weights.size()} instead."
            )
        ...
        # Multiply the attention weights by the value_states
        # attn_output: [Batch_Size, Num_Heads, Num_Patches, Head_Dim]
        attn_output = torch.matmul(attn_weights, value_states)

        if attn_output.size() != (batch_size, self.num_heads, seq_len, self.head_dim):
            raise ValueError(
                f"Expected `attn_output` to have shape {(batch_size, self.num_heads, seq_len, self.head_dim)}, "
                f"but got {attn_output.size()} instead."
            )
